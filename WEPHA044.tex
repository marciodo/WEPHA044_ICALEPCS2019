...
The test was performed for many different lengths. A correlation of 1 should be observed when we plot the measurement of one system against the other. Figures \ref{fig:blen_result_21} and \ref{fig:blen_result_24} shows the scattering with the ATCA system in the x-axis and the VME system in the y-axis. The linear regression result shows that the correlation is close to 1, does confirming that the new system provides the same expected results.
